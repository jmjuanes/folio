export const createServer = (rules = []) => {
    return (middlewares, devServer) => {
        ...
        devServer.app.use((request, response, next) => {
            ...
            // register listeners to read data sent in the request
            const data = [];
            request.on("data", chunk => {
                data.push(chunk.toString());
            });
            request.on("end", () => {
                request.body = data.length > 0 ? JSON.parse(data.join("")) : {};
                next();
            });
            request.on("error", error => {
                console.error("Error reading body data:", error);
                next(error);
            });
        });
