...
const BoardItem = props => {
    ...
    return (
        <a href={`#b/${props.board.id}`} className={itemClass} title={props.board.name}>
...
export const Sidebar = props => (
    <div className="w-64 h-full bg-white shrink-0 flex flex-col justify-between border-r-1 border-gray-200">
        <div className="flex flex-col gap-2 h-full overflow-y-auto overflow-x-hidden">
            <div className="sticky top-0 font-serif font-bold tracking-tighter text-3xl leading-none select-none bg-white pt-5 px-3 pb-3">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    {(props.boards || []).map(item => (
                        <BoardItem
                            ...
                            board={item}
                            active={props.currentId === item.id}
                            onDelete={event => {
                    ...
                    {props.boards && props.boards.length === 0 && (
                        <div className="bg-gray-50 rounded-lg p-6 border-0 border-gray-200">
                            ...
                            <div className="text-center font-bold text-gray-700 text-sm mb-1">No boards available</div> 
                            <div className="text-center text-xs text-gray-500">Your created boards will be displayed here.</div>
                        </div>
                    )}
                </div>
            </div>
        ...
        <div className="px-3 pt-3 pb-3 bg-white">
            ...
            <ActionButton icon="logout" text="Sign out" onClick={props.onLogout} />
        </div>
    </div>
);
