...
const fixPagesIndex = pages => {
    ...
    // 1. First we would need to sort pages by the current index
    // and then assign the new correct index by its position on the list
    pages
        .map(page => ({id: page.id, index: page.index}))
        .sort((a, b) => a.index - b.index)
        .forEach((page, index) => {
            pagesMap.set(page.id, index);
        });
    // 2. Iterate over pages and fix the assigned index
    pages.forEach(page => {
        page.index = pagesMap.get(page.id);
    });
};
