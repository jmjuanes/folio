...
export const getSceneStateFromInitialData = initialData => ({
    ...
    translateX: initialData?.translateX || 0,
    translateY: initialData?.translateY || 0,
    zoom: initialData?.zoom || ZOOM_DEFAULT,
});
...
export const createScene = initialData => {
    const scene = {
        ...
        getErasedElements: () => {
            return scene.elements.filter(element => element.erased);
        },
        ...
        addTextElement: (text, tx = null, ty = null) => {
            scene.clearSelection();
            const x = tx ?? (scene.translateX + scene.width / 2);
            const y = ty ?? (scene.translateY + scene.height / 2);
        ...
        addHistory: entry => {
            ...
            if (scene.historyIndex > 0) {
                scene.history = scene.history.slice(scene.historyIndex);
                scene.historyIndex = 0;
            }
            ...
            scene.history.unshift(entry);
        },

        // @description Perform an undo change to the scene
        undo: () => {
            if (scene.historyIndex < scene.history.length) {
                const entry = scene.history[scene.historyIndex];
                ...
                scene.historyIndex = scene.historyIndex + 1;
            }
        },

        // @description Perform a redo action to the scene
        redo: () => {
            if (scene.historyIndex > 0 && scene.history.length > 0) {
                scene.historyIndex = scene.historyIndex - 1;
                const entry = scene.history[scene.historyIndex];
                if (entry.type === CHANGES.CREATE) {
                    entry.elements.forEach(el => {
        ...
        canUndo: () => {
            return !(scene.historyIndex >= scene.history.length);
        },

        // @description Check if redo action is enabled
        canRedo: () => {
            return !(scene.historyIndex === 0 || scene.history.length < 1);
        },
        ...
        setZoom: (value = ZOOM_DEFAULT) => {
            ...
            const {translateX, translateY} = getTranslateCoordinatesForNewZoom(newZoom, scene);
            // Update scene values
            scene.zoom = newZoom;
        ...
        pasteElementsFromClipboard: (event, point) => {
            const x = point ? (point.x - scene.translateX) / scene.zoom : null;
            const y = point ? (point.y - scene.translateY) / scene.zoom : null;
