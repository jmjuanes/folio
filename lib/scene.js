...
import {getElementConfig, createNewElement} from "@lib/elements.js";
...
export const sceneActions = {
    ...
    //
    setZoom: (scene, value) => {
        const prevZoom = scene.zoom;
        const nextZoom = Math.max(ZOOM_MIN, Math.min(ZOOM_MAX, value));
        scene.translateX = Math.floor(scene.translateX + scene.width * (prevZoom - nextZoom) / 2);
        scene.translateY = Math.floor(scene.translateY + scene.height * (prevZoom - nextZoom) / 2);
        scene.zoom = nextZoom;
    },
    zoomIn: (scene) => {
        return sceneActions.setZoom(scene, scene.zoom + ZOOM_STEP);
    },
    zoomOut: (scene) => {
        return sceneActions.setZoom(scene, scene.zoom - ZOOM_STEP);
    },
