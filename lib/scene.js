...
const setElementsOrderInScene = (scene, elements, sign, absolute) => {
    ...
    const minOrder = Math.min(...scene.page.elements.map(el => el[FIELDS.ORDER]));
    const maxOrder = Math.max(...scene.page.elements.map(el => el[FIELDS.ORDER]));
        ...
        .forEach((element, index) => {
            ...
            // move only individual elements
                ...
                if (minOrder <= newPosition && newPosition <= maxOrder) {
                    ...
                    if (nextElementGroup) {
                        while((minOrder <= newPosition + sign) && (newPosition + sign <= maxOrder) && scene.page.elements[newPosition + sign]?.group === nextElementGroup) {
