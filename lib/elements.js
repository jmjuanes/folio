...
export const elementsConfig = {
    ...
    [ELEMENTS.ARROW]: {
        ...
        getSnapEdges: element => {
            return [
                {x: element.x1, points: [[element.x1, element.y1]]},
                {x: element.x2, points: [[element.x2, element.y2]]},
                {y: element.y1, points: [[element.x1, element.y1]]},
                {y: element.y2, points: [[element.x2, element.y2]]},
            ];
        },
        initialize: values => {
            return {
...
export const getElementsSnappingEdges = (elements) => {
    const edges = elements.map(el => {
        ...
        if (typeof elementsConfig[el.type].getSnapEdges === "function") {
            return elementsConfig[el.type].getSnapEdges(el);
        }
        // Generate the default edges
        return [
            {x: el.x1, points: [[el.x1, el.y1], [el.x1, el.y2]]},
            {x: el.x2, points: [[el.x2, el.y1], [el.x2, el.y2]]},
            {y: el.y1, points: [[el.x1, el.y1], [el.x2, el.y1]]},
            {y: el.y2, points: [[el.x1, el.y2], [el.x2, el.y2]]},
        ];
    });
    return edges.flat();
};
