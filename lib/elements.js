    ...
    [ELEMENTS.ARROW]: {
        ...
        getBounds: element => {
            ...
            if (typeof element.xCenter === "number") {
                bounds.push({
                    path: getCurvePath([element.x1, element.y1], [element.xCenter, element.yCenter], [element.x2, element.y2]),
                });
            }
            else {
                bounds.push({
                    path: getCurvePath([element.x1, element.y1], [element.x2, element.y2]),
                });
            }
        ...
        initialize: values => {
            return {
                ...
                [FIELDS.START_BINDING]: null,
                [FIELDS.END_BINDING]: null,
            };
        },
...
// @public get intersection point between two elements
// @param {object} a: arrow element
// @param {object} b: element to calculate the intersection
export const getElementsIntersection = (a, b) => {
    const isCurvedArrow = typeof a.xCenter === "number";
    const segments = [];
    // Calculate bound segments of the second element
    if (true) {
        segments.push([[b.x1, b.y1], [b.x2, b.y1]]);
        segments.push([[b.x2, b.y1], [b.x2, b.y2]]);
        segments.push([[b.x2, b.y2], [b.x1, b.y2]]);
        segments.push([[b.x1, b.y2], [b.x1, b.y1]]);
    }
    // Find the intersection point
    for (let i = 0; i < segments.length; i++) {
        const s = segments[i];
        if (isCurvedArrow) {
            const points = getQuadraticCurveIntersection([a.x1, a.y1], [a.xCenter, a.yCenter], [a.x2, a.y2], s[0], s[1]);
            if (points && points.length > 0) {
                return points[0];
            }
        }
        // If arrow is not curved, calculate intersection between segments
        else {
            const point = getSegmentsIntersection([a.x1, a.y1], [a.x2, a.y2], s[0], s[1]);
            if (point) {
                return point;
            }
        }
