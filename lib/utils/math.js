...
// @description split a bezier curve
export const splitBezierCurve = (s, c, e, t1, t2 = null) => {
    let p = [s, c, e];
    const q = [s, c, e];
    while (p.length > 1) {
        const r = [];
        for (let i = 0; i < p.length - 1; i++) {
            const pt = lerp(p[i], p[i + 1], t1);
            q.push(pt);
            r.push(pt);
        }
        p = r;
    ...
    // Check if the t2 value has been defined
    if (typeof t2 !== "number") {
        return result;
    }
    // We need to split again the generated bezier curve
    return [
        result[0],
        ...splitBezierCurve(result[1][0], result[1][1], result[1][2], (t2 - t1) / (1 - t2)),
    ];
};

// @description convert a point of the bezier curve into a t value
export const convertBezierPointToTValue = (s, c, e, p) => {
    const x = [p[0] - c[0], p[1] - c[1]];
    const a = [s[0] - c[0], s[1] - c[1]];
    const b = [e[0] - c[0], e[1] - c[1]];
    return Math.sqrt((- x[0] * a[1] + x[1] * a[0]) / (a[0] * b[1] - a[1] * b[0]));
};
