...
export const getSegmentsIntersection = (a, b) => {
    // Check if none of the lines are of length 0
    if ((a[0][0] === a[1][0] && a[0][1] === a[1][1]) || (b[0][0] === b[1][0] && b[0][1] === b[1][1])) {
        return false
    } 
    // Calculate denominator to check if lines are parallel
    const d = ((b[1][1] - b[0][1]) * (a[1][0] - a[0][0])) - ((b[1][0] - b[0][0]) * (a[1][1] - a[0][1]));
    if (d === 0) {
        return false
    }
    // Calculate the intersection point
    // let ua = ((x4 - x3) * (y1 - y3) - (y4 - y3) * (x1 - x3)) / d;
    const ua = ((b[1][0] - b[0][0]) * (a[0][1] - b[0][1]) - (b[1][1] - b[0][1]) * (a[0][0] - b[0][0])) / d;
    // let ub = ((x2 - x1) * (y1 - y3) - (y2 - y1) * (x1 - x3)) / d;
    const ub = ((a[1][0] - a[0][0]) * (a[0][1] - b[0][1]) - (a[1][1] - a[0][1]) * (a[0][0] - b[0][0])) / d;
