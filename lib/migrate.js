...
export const migratePages = (pages, oldElements, version) => {
    if (!pages && oldElements) {
        return [{
            id: generateRandomId(),
            title: "Page 0",
            elements: migrateElements(oldElements, version),
        }];
    }
    // migrate pages
...
export const migrate = (data, version = "2") => {
    return {
        ...
        pages: migratePages(data?.pages, data?.elements, data?.version || version),
