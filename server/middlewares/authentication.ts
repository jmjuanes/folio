...
export const authentication = async (ctx: Context, next: () => Promise<any>) => {
    ...
    if (!token) {
        return sendErrorResponse(ctx, HTTP_CODES.UNAUTHORIZED, API_ERROR_MESSAGES.AUTHENTICATION_REQUIRED);
    }
    ...
    if (!tokenPayload?.username) {
        return sendErrorResponse(ctx, HTTP_CODES.FORBIDDEN, API_ERROR_MESSAGES.INVALID_TOKEN);
    }
